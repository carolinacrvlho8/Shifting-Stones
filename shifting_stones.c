...
void print_board() {
    ...
    printf("\n(Quadrado colorido mostra cor visível; número indica posição 1..9)\n");
}
...
void autosave() {
    ...
    if (!f) { printf("Aviso: não foi possível gravar jogo.\n"); return; }
...
void player_turn(int p) {
    ...
    while (1) {
        ...
        if (op[0] == 'a') {
            if (players[p].hand_count == 0) { printf("Não tens cartas para descartar.\n"); continue; }
            printf("Escolhe índice da carta a descartar (1..%d): ", players[p].hand_count);
            int ci; if (scanf("%d%*c", &ci) != 1) { printf("Entrada inválida\n"); while (getchar() != '\n'); continue; }
            if (ci < 1 || ci > players[p].hand_count) { printf("Índice inválido\n"); continue; }
            ...
            if (a < 1 || a > 9 || b < 1 || b > 9) { printf("Posições inválidas\n"); continue; }
            if (!adjacent(a - 1, b - 1)) { printf("Apenas adjacentes (não diagonais)\n"); continue; }
            ...
            if (pos < 1 || pos > 9) { printf("Posição inválida\n"); continue; }
            ...
            if (players[p].hand_count == 0) { printf("Não tens cartas para pontuar.\n"); continue; }
            ...
            if (card_matches_board(&players[p].hand[ci - 1])) {
                ...
                printf("Carta não corresponde ao tabuleiro.\n");
            }
            continue;
        }
        else if (op[0] == 'd') {
    ...
    while (1) {
        ...
        if (check_endgame(&winner_idx)) {
            if (winner_idx == round_first_player) {
                printf("Jogador %s pontuou %d cartas e activa fim. O adversário ainda joga o seu turno final.\n",
...
void show_rules() {
    ...
    printf("- Cada jogador começa com 4 cartas. Cada carta é um padrão (2..4 pedras) e pontos.\n");
    printf("- No turno, podes: descartar 1 carta para trocar pedras adjacentes; descartar 1 carta para virar pedra;\n");
    printf("  pontuar carta (se o padrão estiver visível na orientação correta); terminar turno (comprar até 4);\n");
    printf("  passar a vez (+2 cartas), não é permitido passar duas vezes seguidas.\n");
